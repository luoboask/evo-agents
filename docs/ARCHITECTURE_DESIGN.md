...
┌─────────────────────────────────────────────────────────┐
│                  数据存储层 (隔离)                       │
│  ~/.openclaw/workspace-ai-baby-config/   # main 数据    │
│  ~/.openclaw/workspace-baby1-config/     # baby1 数据   │
│  ~/.openclaw/workspace-baby2-config/     # baby2 数据   │
└─────────────────────────────────────────────────────────┘
```

### 核心原则

1. **单一 Workspace** - 所有 Agent 共享同一个 workspace
2. **技能共享** - skills/ 目录所有 Agent 共用
3. **配置集中** - config/agents.yaml 管理所有 Agent
