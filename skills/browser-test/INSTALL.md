# browser-test - 浏览器测试技能安装指南

**使用 Puppeteer 进行网页测试、截图、获取控制台日志。**

> browser-debug 的简化版，专注于基础测试功能
...
```bash
# 1. 进入技能目录
cd ~/.openclaw/workspace/skills/browser-test

# 2. 安装依赖
npm install

# 3. 测试安装
node test.js http://example.com
```
...
```
~/.openclaw/workspace/skills/browser-test/
├── SKILL.md              # 技能定义
├── INSTALL.md            # 本文件
├── package.json          # npm 配置
...
```bash
# 访问页面并截图
node test.js open http://localhost:3000/category --screenshot

# 获取控制台日志
node test.js logs http://localhost:3000/category
...
# 2. 截图测试
node test.js open http://example.com --screenshot
...
```bash
chmod -R 755 ~/.openclaw/workspace/skills/browser-test
```
...
### Q: browser-test 和 browser-debug 有什么区别？

**A:** 

| 功能 | browser-test | browser-debug |
|------|--------------|---------------|
| 页面访问 | ✅ | ✅ |
| 截图 | ✅ | ✅ |
| 控制台日志 | ✅ | ✅ |
...
**建议：** 基础测试用 browser-test，深度调试用 browser-debug。
...
```bash
rm -rf ~/.openclaw/workspace/skills/browser-test
```
...
1. 运行 `node test.js http://example.com` 检查安装
